...
try:
    ...
    if API_URL != "https://api.openai.com/v1/chat/completions":
        ...
        print("Warning！API_URL構成オプションは廃止されます，API_URL_REDIRECT構成に変更してください")
except:
    pass
# 新しいバージョンの設定
    ...
    try:
        ...
        model_info.update({
            "yi-34b-chat-0205": {
                ...
                "can_multi_thread": False,  # 目前来言う，#情况下并发量极低，因此禁用
...
# -=-=-=-=-=-=- azure模型对齐支持 -=-=-=-=-=-=-
AZURE_CFG_ARRAY = get_conf("AZURE_CFG_ARRAY") # <-- 用于定义and切换多piecesazure模型 -->
...
def predict_no_ui_long_connection(inputs, llm_kwargs, history, sys_prompt, observe_window=[], console_slience=False):
    ...
    n_model = 1
    if '&' not in model:
        assert not model.startswith("tgui"), "TGUIは関数プラグインの実装をサポートしていません"

        # 1つの大規模言語モデルにのみ質問する場合：
        method = model_info[model]["fn_without_ui"]
        return method(inputs, llm_kwargs, history, sys_prompt, observe_window, console_slience)
    else:

        # 如果同時InquiryMultipleLargeLanguageModels，这pieces稍微啰嗦一点，但思路相同，您不必读这pieceselse分支
        ...
        def mutex_manager(window_mutex, observe_window):
            while True:
                ...
                for i in range(n_model):
                    ...
                    chat_string.append( f"【{str(models[i])} 言う】: <font color=\"{color}\"> {window_mutex[i][0]} </font>" )
                res = '<br/><br/>\n\n---\n\n'.join(chat_string)
                # # # # # # # # # # #
        ...
        for i, future in enumerate(futures):  # wait and get
            color = colors[i%len(colors)]
            return_string_collect.append( f"【{str(models[i])} 言う】: <font color=\"{color}\"> {future.result()} </font>" )
...
def predict(inputs, llm_kwargs, *args, **kwargs):
    ...
    inputs = apply_gpt_academic_string_mask(inputs, mode="show_llm")
    method = model_info[llm_kwargs['llm_model']]["fn_with_ui"]  # ここでエラーが発生した場合，configのAVAIL_LLM_MODELSオプションを確認する
