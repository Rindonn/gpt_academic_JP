prompts_hs = """ 请以“{headstart}”为开头，编写一pieces小言うの一幕。

- 尽量短，不要包含太多情节，因为你次に置き換える会与用户互动续写下面的情节，要留出足够的互动空间。
- 出现人物時，给出人物的名字。
- 积极地运用环境描写、人物描写等手法，让读者能够感受到你的故事世界。
- 积极地运用修辞手法，比如比喻、拟人、排比、对偶、夸张等等。
- 字数要求：第一幕的字数少于300字，且少于2pieces段落。
"""

prompts_interact = """ 小言う的前文回顾：
「
{previously_on_story}
」

你是一pieces作家，根据以上的情节，给出4种不同的后续剧情发展方向，每pieces发展方向都精明扼要地用一句话言う明。稍后，我置き換える在这4pieces选择中，挑选一种剧情发展。
...
prompts_resume = """小言う的前文回顾：
「
{previously_on_story}
」

你是一pieces作家，私たちは正在互相讨论，确定后续剧情的发展。
在以下的剧情发展中，
「
{choice}
」
我认为更合理的是：{user_choice}。
请在前文的基础上（不要重复前文），围绕我选定的剧情情节，编写小言う的下一幕。
...
- 积极地运用环境描写、人物描写等手法，让读者能够感受到你的故事世界。
- 积极地运用修辞手法，比如比喻、拟人、排比、对偶、夸张等等。
- 小言う的下一幕字数少于300字，且少于2pieces段落。
"""


prompts_terminate = """小言う的前文回顾：
「
{previously_on_story}
」

你是一pieces作家，私たちは正在互相讨论，确定后续剧情的发展。
今，故事该終了する了，我认为最合理的故事结局是：{user_choice}。

请在前文的基础上（不要重复前文），编写小言う的最後に一幕。

- 不要重复前文。
- 出现人物時，给出人物的名字。
- 积极地运用环境描写、人物描写等手法，让读者能够感受到你的故事世界。
- 积极地运用修辞手法，比如比喻、拟人、排比、对偶、夸张等等。
- 字数要求：最後に一幕的字数少于1000字。
"""
...
class MiniGame_ResumeStory(GptAcademicGameBaseState):
    story_headstart = [
        '先行者知道，他今是全宇宙中唯一的一pieces人了。',
        '深夜，一pieces年轻人穿过天安门广场向纪念堂走去。在二十二世纪编年史中，计算机把他的代号定为M102。',
        '他知道，这最後に一课要提前讲了。又一阵剧痛从肝部袭来，几乎使他晕厥过去。',
        '在距地球五万光年的远方，在银河系的中心，一场延续了两万年的星际战争已接近尾声。那里的太空中渐渐隐现出一pieces方形区域，仿佛灿烂的群星的背景被剪出一pieces方口。',
        '伊依一行三人乘坐一艘游艇在南太平洋上做吟诗航行，他们的目的地是南极，如果几天后能顺利到达那里，他们置き換える钻出地壳去看诗云。',
        '很多人生来就会莫名其妙地迷上一样东西，仿佛他的出生就是要and这东西约会似的，正是这样，圆圆迷上了肥皂泡。'
    ]
    ...
    def begin_game_step_0(self, prompt, chatbot, history):
        ...
        self.sys_prompt_ = '你是一pieces想象力丰富的杰出作家。正在与你的朋友互动，一起写故事，因此你每次写的故事段落应少于300字（结局除外）。'


    def generate_story_image(self, story_paragraph):
        try:
            from crazy_functions.ImageGeneration import gen_image
            prompt_ = predict_no_ui_long_connection(inputs=story_paragraph, llm_kwargs=self.llm_kwargs, history=[], sys_prompt='你需要根据用户给出的小言う段落，进行简短的环境描写。要求：80字以内。')
    ...
    def step(self, prompt, chatbot, history):
        ...
        if self.step_cnt == 0:
            ...
            if prompt.strip() == 'exit' or prompt.strip() == '終了する剧情':
                # should we terminate game here?
                self.delete_game = True
                yield from update_ui_lastest_msg(lastmsg=f"游戏終了する。", chatbot=chatbot, history=history, delay=0.)
                return
            if '剧情收尾' in prompt:
        ...
        if self.cur_task == 'head_start':
            ...
            self.next_choices = yield from request_gpt_model_in_new_thread_with_ui_alive(
                inputs_,
                '请在以下几种故事走向中，选择一种。当然，您也可以给出您心中的其他故事走向。另外，如果您希望剧情立即收尾，请入力剧情走向，并以“剧情收尾”四pieces字ヒント程序。', self.llm_kwargs,
                chatbot,
                history_,
                self.sys_prompt_
            )
