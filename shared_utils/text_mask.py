...
# 这段代码是使用するPython编程语言中的re模块，即正则表达式库，来定义了一pieces正则表达式模式。
...
def apply_gpt_academic_string_mask(string, mode="show_all"):
    """
    当文字列中有掩码tag時（<gpt_academic_string_mask><show_...>），根据文字列要给谁看（大模型，还是web渲染），对文字列进行处理，戻る处理后的文字列
...
def apply_gpt_academic_string_mask_langbased(string, lang_reference):
    """
    当文字列中有掩码tag時（<gpt_academic_string_mask><lang_...>），根据语言，选择ヒント词，对文字列进行处理，戻る处理后的文字列
...
def build_gpt_academic_masked_string_langbased(text_show_english="", text_show_chinese=""):
    """
    根据语言，选择ヒント词，对文字列进行处理，戻る处理后的文字列
    """
